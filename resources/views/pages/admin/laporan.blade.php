...
@section('content')
    ...
    <div class="w-full space-y-5">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            {{-- BARIS 2: FILTER --}}
            <form id="formFilter" method="GET" action="{{ route('admin.laporan.index') }}" class="px-5 py-4">
                        ...
                        @forelse($laporan as $item)
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-4 py-4 border text-center text-black">
                                    {{ $laporan->firstItem() + $loop->index }}</td>
    ...
    @section('script')
        <script>
            function exportLaporan(type) {
                ...
                if (type === 'excel') {
                    window.location.href = "{{ route('admin.laporan.export-excel') }}" + params;
                } else {
